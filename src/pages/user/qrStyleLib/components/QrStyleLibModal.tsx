...
const QrStyleLibModal: React.FC<QrStyleLibModalProps> = ({ isOpen, onClose, onSuccess, item }) => {
    ...
    const displayJson = useMemo(() => {
        ...
        if (displayStyle.logo && displayStyle.logo.length > 50) {
            displayStyle.logo = displayStyle.logo.substring(0, 50) + "...";
        }
    ...
    const executeSave = async () => {
        try {
            ...
            const payload = {
                name,
                styleJson,
                isActive: true, // Always active for user
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-sm overflow-hidden font-primary isolate">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-foreground leading-tight tracking-tight">
                                {item ? "Chỉnh sửa phong cách" : "Thêm phong cách mới"}
                            </h3>
                            <p className="text-[11px] text-foreground-muted font-bold uppercase tracking-widest opacity-70">Thư viện cá nhân</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-1 min-h-0 flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-border/30 overflow-hidden">
                        ...
                        {showRawJson && (
                            <div className="absolute inset-0 z-30 bg-surface/95 backdrop-blur-xl p-8 animate-in slide-in-from-bottom-5 duration-500 overflow-auto">
                                <div className="flex justify-between items-center mb-6">
                                    <span className="text-sm font-black text-primary tracking-widest uppercase">Style Config JSON</span>
                    ...
                    <div className="flex-[55%] flex flex-col overflow-y-auto p-10 custom-scrollbar bg-surface/10">
                        ...
                        <div className="mb-10 w-full animate-in fade-in slide-in-from-right-2 duration-500">
                            <label className={SECTION_LABEL_CLASS}><LayoutGrid size={13} /> Tên phong cách thiết kế</label>
                            <div className="relative group">
                                <input
                                    ...
                                    placeholder="Ví dụ: Phong cách Studio Dark..."
                                    value={name}
                    ...
                    <div className="flex gap-4">
                        {item && (
                            <Button
                                ...
                                disabled={loading}
                            >
                                <Trash2 className="w-4 h-4 mr-2" /> Xóa phong cách
                            </Button>
                        )}
                    </div>
                    <div className="flex gap-4">
            ...
            <ActionConfirmModal
                ...
                title="Xác nhận xóa phong cách"
                description={`Bạn có chắc muốn xóa style "${name}" khỏi thư viện cá nhân?`}
            ...
            <ActionConfirmModal
                ...
                title={item ? "Cập nhật phong cách" : "Tạo phong cách mới"}
                description={`Lưu lại các thay đổi thiết kế cho phong cách "${name}"?`}
                loading={loading}
